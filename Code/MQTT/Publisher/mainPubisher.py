...
#GPIO
LIGHTSENSOR_INPUT_PIN = 23



# Event handler

def callback_func(channel): #LightSensor Callback
    if GPIO.input(23) == 0:
...
def init(): #sent on GPIO state change
    ...
    GPIO.setup(LIGHTSENSOR_INPUT_PIN, GPIO.IN)  # sets mode of specific pin
    GPIO.add_event_detect(LIGHTSENSOR_INPUT_PIN, GPIO.BOTH,
                          callback=callback_func, bouncetime=20)  # event listener on LIghtSensor, bounce time = time to sleep till next event handle
